...
LOOKBACK = 15        # 广度回看交易日
LOCK_DAYS = 21       # 锁定交易日(信号B: 广度SMA30锁定, 每日检查的冷却期; 勿与回测SURGE的14天混淆)
...
def main():
    ...
    if trig and not state.get("active"):
        ...
        print(f"[{today}] 🚨🚨 SURGE 触发! 广度 {b_prev:.0%}→{b_now:.0%}, SMA30分 {s30:.3f}, "
              f"SMA250分 {base:.3f} —— 制度强制 RISKON, 锁定至 {expire_date}")

        send_feishu("🚨 广度突破（SMA30锁定）",
                    f"广度 {b_prev:.0%}→{b_now:.0%}（≥2/3站上SMA50）\n"
                    f"SMA30分 {s30:.3f} · SMA250分 {base:.3f}\n"
                    f"**制度强制 RISKON**，锁定至 {expire_date}",
                    "red")
